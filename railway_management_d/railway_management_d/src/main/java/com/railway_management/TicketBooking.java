        ...
        try {
            ...
            for(String line; !(line = reader.readLine()).contains("#"); ) {
                ...
                try{
                    ...
                    // String colName = coachType.toLowerCase() == "sl" ? "booked_sl" : "booked_ac";
                    // query = "select "+colName+" from train where train_num='"+trainNumber+"' and journey_date='"+date+"'";
                    // rs = statement.executeQuery(query);
                    // int counter = 0;
                    // while(rs.next()) counter = rs.getInt(colName);

                    // for(String passenger : passengers) {
                    //     query = "call assign_berth('"+trainNumber+"', '"+date+"', '"+coachType+"', '"+passenger+"', '"+pnr+"', '"+(counter++)+"')";
                    //     statement.executeUpdate(query);
                    // }
                    // counter--;

                    // query = "update train set "+colName+"="+counter+" where train_num="+trainNumber+" and journey_date='"+date+"'";
                    // statement.executeUpdate(query);
